...
(def ^:private re-cookie-value
  (re-pattern (str "\"" (.-source re-cookie-octet) "*\"|"
                   (.-source re-cookie-octet) "*")))

(def ^:private re-cookie
  (re-pattern (str "\\s*(" (.-source re-token) ")=("
                   (.-source re-cookie-value) ")\\s*[;,]?")))
...
(defn- set-cookies [response encoder]
  (if-let [cookies (:cookies response)]
    (update-in response
               [:headers "Set-Cookie"]
               (fn [existing]
                 (into (if (string? existing) [existing] (vec existing))
                       (write-cookies cookies encoder))))
    response))
...
(defn wrap-cookies
  ...
  :expires   - a date string or js/Date at which the cookie will expire
  ...
  ([handler options]
   (fn [request]
     (async/then (async/call handler (cookies-request request options))
