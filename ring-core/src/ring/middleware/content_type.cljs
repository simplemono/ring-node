...
(defn- body-file-path
  "The .path property of a file stream body (see ring.util.response
  file-response), if present."
  [body]
  (when (some? body)
    (let [p (unchecked-get body "path")]
      (when (string? p) p))))

(defn- guess-mime-type [{:keys [uri]} {:keys [body]} mime-types]
  (or (ext-mime-type uri mime-types)
      (some-> (body-file-path body) (ext-mime-type mime-types))))
  ...
  ([handler options]
   (fn [request]
     (async/then (async/call handler request)
                 #(content-type-response % request options)))))
