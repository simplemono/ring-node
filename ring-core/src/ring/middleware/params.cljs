...
(defn- ^:async read-body [body encoding]
  (cond
    (string? body)             body
    (io/readable-stream? body) (await (io/read-stream-string body encoding))
    (seq? body)                (apply str body)
    :else                      nil))

(defn ^:async assoc-form-params
  "Parse and assoc parameters from the request body with the request. Returns
  a js/Promise of the updated request, as the body is read asynchronously."
  [request encoding]
  (let [params (if-let [body (and (req/urlencoded-form? request)
                                  (:body request))]
                 (parse-params (or (await (read-body body encoding)) "") encoding)
                 {})]
    (-> request
        (assoc-param-map :form-params params)
        (assoc-param-map :params params))))

(defn- ^:async params-request* [request options]
  (let [encoding (or (:encoding options)
                     (req/character-encoding request)
                     "UTF-8")
        request  (if (:form-params request)
                   request
                   (await (assoc-form-params request encoding)))]
...
(defn params-request
  ...
  map. Returns a js/Promise of the updated request. See: wrap-params."
  ([request]
...
(defn wrap-params
  ...
  ([handler options]
   (fn [request]
     (.then (params-request* request options) handler))))
